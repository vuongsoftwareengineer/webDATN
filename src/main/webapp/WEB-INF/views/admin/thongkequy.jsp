...
<head>
<meta charset="utf-8">
<title>DANH SÁCH SẢN PHẨM BÁN ĐƯỢC TRONG QUÝ</title>
...
<h2 style="margin-left: 250px; font-style: italic; color: #8B4513"> Xin chào ${sessionScope.admin.getHoTen()}! </h2>
  <p style="margin-left: 250px; color: brown">Chào mừng bạn quay trở lại với trang quản trị của Website </p>
		...
		<div class="container">
			...
			<h2 style="font-weight: bold;color: #FF6666">DANH SÁCH SẢN PHẨM ĐÃ BÁN ĐƯỢC TRONG QUÝ</h2>
			<div>
    ...
    <tbody>
    <c:forEach var="p" items="${thongkequys}">
      <tr>
        <td>${p.id}</td>
        <td>${p.ten}</td>
        <td>${p.quy}</td>
...
<div class="col-md-6 chart-blo-1" style="margin-left: 600px">
    		...
    		<h7 style="margin-left: 160px"><b>Thống kê tổng sản phẩm bán được theo biểu đồ cột</b></h7>
    		 <canvas id="line" height="300" width="700" style="width: 700px; height: 300px;"> </canvas>
    		  <h3 class="tlt">Line Chart</h3>
                                            ...
                                            <script>
                                                var lineChartData = {
                                                    labels: ["",<c:forEach var="item" items="${thongkequys}">'${item.id}',</c:forEach>],
                                                    datasets: [
                                                        {
                                                            fillColor: "#FC8213",
                                                            data: ["0",<c:forEach var="item" items="${thongkequys}">${item.tong},</c:forEach>]
                                                        }
                                                    ]
                                            ...
                                            <script>
                                                var barChartData = {
                                                    ...
                                                    datasets: [
                                                        {
                                                            fillColor: "#FC8213",
                                                            data: [<c:forEach var="item" items="${thongkequys}">${item.tong},</c:forEach>]
                                                        }
                                                    ]
          ...
          <%--    <div class="col-md-6 chart-blo-1" style="margin-left: 600px">
    		...
    		<h7 style="margin-left: 160px"><b>Thống kê tổng sản phẩm bán được theo biểu đồ cột</b></h7>
    		 <canvas id="line" height="300" width="700" style="width: 700px; height: 300px;"> </canvas>
    		 <c:forEach var="v" items="${thongkequys}">
                     <script>
                        var lineChartData = {
						labels : ["",<c:forEach var="item" items="${thongkequys}">'${item.id}',</c:forEach>],
						datasets : [
