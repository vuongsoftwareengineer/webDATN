...
<h2 style="margin-left: 250px; font-style: italic; color: #8B4513"> Xin chào ${sessionScope.admin.getHoTen()}! </h2>
  <p style="margin-left: 250px; color: brown">Chào mừng bạn quay trở lại với trang quản trị của Website </p>
		...
		<div class="container">
			<br>
			<h2 style="font-weight: bold;color: #FF6666">DANH SÁCH THƯƠNG HIỆU</h2>	
			 <a href="admin/themth.html"><i class="far fa-plus-square"></i> Thêm thương hiệu </a><h6 style="color: red">${message}</h6>
			<div>
    ...
    <c:forEach var="p" items="${thuonghieus}">
      <tr>
        ...
        <td width="312px"><img src="./images/${p.anh}" style="width: 100%">
          <td><a href="admin/capnhatth/${p.id}.html" onclick="if(!(confirm('Bạn có chắc chắn muốn sửa thương này không?'))) return false;">Sửa</a></td>
        <td><div><a href="admin/xoath/${p.id}.html" onclick="if(!(confirm('Bạn có chắc chắn muốn thương hiệu này không?'))) return false;">Xóa</a></div></td> 
			</tr>
      </c:forEach>
					</tbody>
				</table>
